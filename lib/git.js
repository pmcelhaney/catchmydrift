...
function hashObject(repositoryRoot, contents) {
  const result = runGit(['hash-object', '--stdin'], repositoryRoot, { input: contents });
  const objectId = result.stdout.toString('utf8').trim();
  if (!/^[0-9a-f]{40}$|^[0-9a-f]{64}$/.test(objectId)) {
    throw new OperationalError('Git returned an invalid blob object ID.');
  }
  return objectId;
}
...
function commitsForPath(repositoryRoot, relativePath) {
  if (!hasHead(repositoryRoot)) {
    return [];
  }
  return runGit(
    ['rev-list', '--topo-order', '--reverse', 'HEAD', '--', relativePath],
    repositoryRoot
...
function parseTreeRecord(record) {
  ...
  if (fields.length !== 3 || !relativePath || !/^[0-9a-f]{40}$|^[0-9a-f]{64}$/.test(fields[2])) {
    throw new OperationalError('Git returned an invalid tree record.');
  }
  return { mode: fields[0], type: fields[1], objectId: fields[2], path: relativePath };
}

function listTreeEntries(repositoryRoot, commit, rootRelative = '.') {
  const result = runGit(
    ['ls-tree', '-r', '-z', commit, '--', rootRelative],
    repositoryRoot
  );
  return splitNullDelimited(result.stdout).map(parseTreeRecord).sort((left, right) => comparePaths(left.path, right.path));
}

function treeEntryForPath(repositoryRoot, commit, relativePath) {
  const result = runGit(
    ['ls-tree', '-z', commit, '--', relativePath],
    repositoryRoot
  );
  const records = splitNullDelimited(result.stdout);
  if (records.length === 0) {
    return null;
  }
  if (records.length !== 1) {
    throw new OperationalError(`Git returned multiple tree entries for path: ${relativePath}`);
  }
  const entry = parseTreeRecord(records[0]);
  return entry.path === relativePath ? entry : null;
}

function readBlob(repositoryRoot, objectId) {
  return runGit(['cat-file', 'blob', objectId], repositoryRoot).stdout;
}

function readFileAtCommit(repositoryRoot, commit, relativePath) {
  const entry = treeEntryForPath(repositoryRoot, commit, relativePath);
  if (entry === null) {
    return null;
  }
  if (entry.type !== 'blob') {
    throw new OperationalError(`Tracked path is not a blob at ${commit}: ${relativePath}`);
  }
  return readBlob(repositoryRoot, entry.objectId);
}

function selectNewerCommit(repositoryRoot, commits) {
  const candidates = [...new Set(commits.filter(commit => commit && commit !== EMPTY_TREE_HASH))];
  ...
  if (!hasHead(repositoryRoot)) {
    throw new OperationalError('Cannot order baseline commits without HEAD.');
  }
  const history = runGit(['rev-list', '--topo-order', 'HEAD'], repositoryRoot)
    .stdout.toString('utf8').split('\n').filter(Boolean);
  for (const commit of history) {
    if (candidateSet.has(commit)) {
      return commit;
    }
  }
  throw new OperationalError('A candidate baseline commit is not reachable from HEAD.');
}
